...
export default class AccountDownloads extends PageManager {
    ...
    onReady() {
        ...
        if (window.accountDownloadsInitialized) {
            console.log("[AccountDownloads] Already initialized, skipping double execution");
            return;
        }
        window.accountDownloadsInitialized = true;
        ...
        if (isDownloadsPage && currentUrl.includes("show=downloads")) {
            ...
            // Hide orders content from view
            $(".account-content").first().hide();
            this.$container.show();
            
            // Change page heading from Orders to Downloads
            const $heading = $(".page-heading").first();
            if ($heading.text().includes("Orders") || $heading.text().includes("Order")) {
        ...
        this.updateNavigationForDownloads();
    ...
    /**
     * Update navigation tab to show Downloads as active instead of Orders
     */
    updateNavigationForDownloads() {
        console.log("[AccountDownloads] Updating active navigation tab");
        // Remove active class from all tabs
        const $navItems = $('.navBar--account .navBar-item');
        if ($navItems.length) {
            $navItems.removeClass('is-active');
            // Find the downloads tab and make it active
            $navItems.each(function() {
                const href = $(this).find('a').attr('href');
                if (href && (href.includes('show=downloads') || href.includes('action=downloads') || href.includes('/downloads'))) {
    ...
    renderDownloads(orders) {
        if (!orders || orders.length === 0) {
            ...
            if (this.context && typeof this.context.lang === "function") {
                const langText = this.context.lang(
                    "account.downloads.no_downloads",
                );
                if (langText && langText !== "account.downloads.no_downloads") {
                    noDownloadsText = langText;
                }
            } else if (
                window.langTranslations &&
                window.langTranslations["account.downloads.no_downloads"]
            ) {
                noDownloadsText =
                    window.langTranslations["account.downloads.no_downloads"];
            }
            ...
            `);
            this.$pagination.hide();
            this.showContent(); // Hide loading spinner
            return;
        }
        ...
        let html = `
            <div class="downloads-table-container">
                <p class="downloads-instructions">
        ...
        // Bind download link clicks to show toast
        this.$list.find('.download-link').off('click.toast').on('click.toast', () => {
            this.showDownloadToast();
        });
    }

    showDownloadToast() {
        let $toast = $('#download-toast-notification');
        if (!$toast.length) {
            $toast = $('<div id="download-toast-notification">Download about to start</div>').css({
                position: 'fixed',
                top: '20px',
                right: '20px',
